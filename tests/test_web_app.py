...
from pact.web.browser_bundle import browser_python_archive
...
def test_browser_core_archive_includes_browser_import_chain() -> None:
    archive = browser_python_archive("core")
    ...
    assert {
        "pact/browser.py",
        "pact/carriers/__init__.py",
        "pact/carriers/c2pa.py",
        "pact/carriers/c2pa_text.py",
        "pact/carriers/structured.py",
        "pact/carriers/text.py",
        "pact/fingerprints.py",
        "pact/watermarks/__init__.py",
        "pact/watermarks/image.py",
    }.issubset(names)


def test_browser_core_archive_imports_without_document_packages(
    tmp_path: Path,
) -> None:
    archive_path = tmp_path / "pact-browser-core.pyz"
    archive_path.write_bytes(browser_python_archive("core"))
    script = f"""
import importlib.abc
import sys


class BlockDocumentPackages(importlib.abc.MetaPathFinder):
    def find_spec(self, fullname, path=None, target=None):
        blocked = ("c2pa", "defusedxml", "PIL", "pypdf", "trustmark")
        if any(
...
sys.meta_path.insert(0, BlockDocumentPackages())
sys.path.insert(0, {str(archive_path)!r})
import pact.browser
print("ok")
"""
...
def test_web_workspace_is_optional_and_serves_pyodide_assets(
    ...
    assert "pact/carriers/c2pa.py" in core_names
    assert "pact/carriers/c2pa_text.py" in core_names
    assert "pact/fingerprints.py" in core_names
    assert "pact/watermarks/image.py" in core_names
    ...
    assert "pact/carriers/c2pa_text.py" in document_names
